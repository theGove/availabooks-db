...
::: {.ai-conversation prompt="Write a SQL statement to list the full_name and nationality of every artist who has created a work of art with the subject \"Portrait.\" Use a nested subquery rather than hard-coded IDs."}
> To do this, you need three levels of query: one to find the work IDs with the subject "Portrait," one to find the artist IDs for those works, and one to look up the artist information for those artist IDs. Here's the SQL statement:
>
> ```sql
