...
::: {.ai-conversation prompt="Write a SQL statement to list the artist_id, full_name, and nationality of every artist who shares a nationality with Claude Monet. Use a self join on the artist table instead of a subquery."}
...
::: {.ai-conversation prompt="Write a SQL statement to list every artist_id along with the number of works each artist has created. Join the artist and work tables and use GROUP BY instead of a correlated subquery."}
